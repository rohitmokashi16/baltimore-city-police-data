...
class CrimeVisualizations:
    ...
    def __init__(self):
        ...
        self.day_of_the_week = ['Sunday', 'Monday', 'Tuesday',
                                'Wednesday', 'Thursday', 'Friday', 'Saturday']
        self.months_of_year = ['January', 'February', 'March', 'April', 'May',
                               'June', 'July', 'August', 'September', 'October', 'November', 'December']
        self.lower_year = 1950
        self.upper_year = 2021
        self.dataset_obj = Preprocessing()
        # print('Hi')
        #print (self.dataset_obj)
    ...
    def day_of_the_week_boxplot(self, groupby="Year", lower_year=2016, upper_year=2020, is_swarm=False, *neighborhood_name):
        ...
        *neighborhood_name: contains name of ONE neighborhood (string as first and only element of tuple)"""
        ...
        dataset = self.dataset_obj.final_dataset[[
            'Year', 'Month_Name', 'Total_Incidents', 'Day', 'Neighborhood']]
        ...
        if len(neighborhood_name) > 0:
            dataset = dataset[dataset['Neighborhood']
                              == neighborhood_name[0].upper()]
            neighborhood_title_string = f" for neighborhood {neighborhood_name[0]}"
        title = "Incidents by Day of the Week grouped by " + \
            ("month" if groupby.lower() == 'month_name' else "year") + \
            neighborhood_title_string
        ...
        if is_swarm:
            ...
            if groupby.lower() == "month_name":
                sns.swarmplot(x="Total_Incidents", y="Day", data=tp, hue=groupby,
                              order=self.day_of_the_week, size=7, hue_order=self.months_of_year)
            else:
                sns.swarmplot(x="Total_Incidents", y="Day", data=tp,
                              hue=groupby, order=self.day_of_the_week, size=7)
            title = title + " with swarmplot of " + \
                ("month" if groupby.lower() == 'month_name' else "year")

        # Generate boxplot and set labels
        bx_plt = sns.boxplot(x="Total_Incidents", y="Day",
                             data=tp, order=self.day_of_the_week)
    ...
    def district_wise_boxplot(self, groupby="Year", lower_year=2016, upper_year=2020, is_swarm=False):
        ...
        self.dataset_updater(lower_year, upper_year)
        dataset = self.dataset_obj.final_dataset[[
            'Year', 'Month_Name', 'Total_Incidents', 'District']]
        ...
        if is_swarm:
            # Check for month or year grouping and set hue and title accordingly
            if groupby.lower() == "month_name":
                sns.swarmplot(x="Total_Incidents", y="District", data=tp,
                              hue=groupby, size=7, hue_order=self.months_of_year)
            else:
                sns.swarmplot(x="Total_Incidents", y="District",
                              data=tp, hue=groupby, size=7)
    ...
    def indoor_outdoor_crimes_trends(self, groupby="Year", lower_year=2016, upper_year=2020, *neighborhood_name):
        ...
        *neighborhood_name: contains name of ONE neighborhood (string as first and only element of tuple)"""
        ...
        self.dataset_updater(lower_year, upper_year)
        dataset = self.dataset_obj.final_dataset[[
            'Year', 'Month_Number', 'WeekNumber', 'Total_Incidents', 'Inside_Outside', 'Neighborhood']]
        ...
        tp = dataset.groupby(
            by=[groupby, 'Inside_Outside']).count().reset_index()

        # Generate lineplot
        ax = sns.lineplot(x=groupby, y='Total_Incidents',
                          data=tp, hue="Inside_Outside", sort=False)
        fontsz = 15
        ...
        if groupby.lower() == 'year':
            ...
            if groupby.lower() == 'month_number':
                ...
                upper, labels, grouping, fontsz = 54, [
                    i for i in range(1, 54)], "Week Number", 10
        ...
        ax.set_ylim([tp['Total_Incidents'].min()//2,
                    tp['Total_Incidents'].max()*1.25])
        ax.set_ylabel("Total Incidents", fontsize=15)
        ax.set_title(
            f"Trends of Indoors and Outdoors Crimes by {grouping}{neighborhood_title_string}")
        plt.savefig(self.string_bytes, format='jpg')
        self.string_bytes.seek(0)
        plot_base64data = base64.b64encode(self.string_bytes.read())
        return plot_base64data
    ...
    def district_crime_bar_charts(self, lower_year=2016, upper_year=2020, inside_outside_flag=False):
        ...
        self.dataset_updater(lower_year, upper_year)
        dataset = self.dataset_obj.final_dataset[[
            'District', 'Total_Incidents', 'Inside_Outside', 'Year']]
        ...
        if not inside_outside_flag:
            ...
            ax = sns.barplot(data=tp, x="Total_Incidents", y='District', order=tp.sort_values(
                "Total_Incidents", ascending=True).District)
            ...
            tp = dataset.groupby(
                by=["District", "Inside_Outside"]).count().reset_index()
            tp1 = tp.groupby(['District']).sum().sort_values(
                ['Total_Incidents'], ascending=False).reset_index()
            sort_order = {i: 0 for i in tp1['District'].unique()}
            val = 0
            for i in sort_order.keys():
                sort_order[i] = val
                val += 1
            tp = tp.assign(sort=lambda tp: tp['District'].map(
                sort_order)).sort_values('sort').drop('sort', axis=1)
            b1 = plt.barh(tp['District'].unique(
            ), tp.loc[tp['Inside_Outside'] == 'Outside', 'Total_Incidents'])
            b2 = plt.barh(tp['District'].unique(), tp.loc[tp['Inside_Outside'] == 'Inside',
                          'Total_Incidents'], left=tp.loc[tp['Inside_Outside'] == 'Outside', 'Total_Incidents'])
            plt.legend([b1, b2], ["Outside", "Inside"],
                       title="Crime Setting", loc="upper right")
            plt.xlabel("District")
            plt.ylabel("Incident Count")
            plt.title(
                "Stacked Bar Chart of Incident Count by Districts broken into Indoors and Outdoors Incidents")
        plt.savefig(self.string_bytes, format='jpg')
        self.string_bytes.seek(0)
        plot_base64data = base64.b64encode(self.string_bytes.read())
        return plot_base64data

    def hours_of_day_table(self, df):
    ...
    def crimes_per_hour_viz(self, categories, lower_year, upper_year, static):

        # Update dataset based on year filters
        self.dataset_updater(lower_year, upper_year)
        dataset = self.dataset_obj.final_dataset
        data = self.hours_of_day_table(dataset)

        fig = go.Figure()
        if static:
            # Combining all categories into 3 categories
            categories = ["Theft, Larceny & Robbery",
                          "Assault, Homicide & Rape", "Arson & Shooting"]
            data["Assault, Homicide & Rape"] = data["AGG. ASSAULT"] + data["COMMON ASSAULT"] + data["RAPE"]
            data["Arson & Shooting"] = data["SHOOTING"] + data["ARSON"]
            data["Theft, Larceny & Robbery"] = data["AUTO THEFT"] + data["BURGLARY"] + data["LARCENY"] + data["LARCENY FROM AUTO"] + \
                data["ROBBERY - CARJACKING"] + data["ROBBERY - COMMERCIAL"] + \
                data["ROBBERY - RESIDENCE"] + data["ROBBERY - STREET"]
...
def main():
    ...
    lower, upper = map(int, input(
        "Enter the lower year bound and the upper year bound (separated by a space): ").split())
    ...
    nightingale_categories_chart = crime_viz.crimes_per_hour_viz(
        ["AGG. ASSAULT", "COMMON ASSAULT", "RAPE", "SHOOTING", "ARSON"], lower, upper, False)
    #crimes_per_hour_viz(["AGG. ASSAULT","COMMON ASSAULT", "RAPE", "SHOOTING", "ARSON"], False, lower, upper)

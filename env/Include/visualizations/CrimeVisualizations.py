...
class CrimeVisualizations:
	...
	def day_of_the_week_boxplot(self, groupby = "Year", lower_year = 2016, upper_year = 2020, is_swarm = False):
		...
		dataset = self.dataset_obj.final_dataset[['Year', 'Month_Name', 'Total_Incidents', 'Day']]
	...
	def indoor_outdoor_crimes_trends(self, groupby = "Year", lower_year = 2016, upper_year = 2020):
		...
		self.seaborn_plot_settings()
		self.dataset_updater(lower_year, upper_year)
		dataset = self.dataset_obj.final_dataset[['Year', 'Month_Number','WeekNumber', 'Total_Incidents', 'Inside_Outside']]
...
def test():
	...
	# crime_viz.day_of_the_week_boxplot("Year", lower, upper, False)
	# print(len(dotw_year_swarm))
